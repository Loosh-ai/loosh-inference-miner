...
from miner.network.InferenceSynapse import InferenceSynapse, inference, blacklist, priority
...
class BittensorNode:
    ...
    def __init__(self, config=None, log_level="debug"):
        ...
        # Config should be passed from create_node which creates a compatible config
        self.config = config
        self._axon = None
        self._dendrite = None
        self.hotkey = None
    ...
    def setup_axon(self, ip=None, port=8099, external_ip=None, external_port=None):
        ...
        # Handle both bittensor config and our custom config
        if hasattr(self.config, 'bt_config'):
            wallet = bt.wallet(
                name=self.config.wallet_name,
                hotkey=self.config.hotkey_name
            )
        elif hasattr(self.config, 'wallet'):
            wallet = bt.wallet(
...
def create_node(config = None) -> BittensorNode:
    ...
    if config is None:
        ...
        # Create a compatible config object that has the fields bittensor_node expects
        class NodeConfig:
            def __init__(self, miner_config):
                self.netuid = miner_config.netuid
                self.subtensor_network = miner_config.subtensor_network
                self.subtensor_address = miner_config.subtensor_address
                self.wallet_name = miner_config.wallet_name
                self.hotkey_name = miner_config.hotkey_name
                # Also create bittensor config for internal use
                import bittensor as bt
                self.bt_config = bt.config()
                self.bt_config.netuid = miner_config.netuid
                self.bt_config.subtensor.network = miner_config.subtensor_network
                self.bt_config.subtensor.chain_endpoint = miner_config.subtensor_address
                self.bt_config.wallet.name = miner_config.wallet_name
                self.bt_config.wallet.hotkey = miner_config.hotkey_name
        config = NodeConfig(miner_config)
